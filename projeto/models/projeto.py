...
class Projeto(models.Model):
    ...
    titulo_portugues = models.CharField(max_length=256, validators=[
                                        MinLengthValidator(5)], blank=True)
    titulo_ingles = models.CharField(max_length=256, validators=[
                                     MinLengthValidator(5)], blank=True)
    # Sigla: padrão embrapa - máximo 8 dígitos, upcaser, sem caracteres
    # especiais
    sigla = models.CharField(max_length=32, validators=[
                             MinLengthValidator(2)], blank=True)
    ...
    # Líder: são os usuários cadastrados no sistema. Relação: Líder 1 - *
    # Projeto
    lider = models.ForeignKey(
        User, null=True, blank=True, related_name='lider')

    # Instituição: podem ser proponentes ou executoras. Relação: Instituição 1
    # - * Projeto
    ...
    # Projeto relacionado: é um auto-relacionamento, uma vez que um projeto
    # cadastrado pode estar relacionado com outro. Projeto Relacionado 1 - *
    # Projeto
