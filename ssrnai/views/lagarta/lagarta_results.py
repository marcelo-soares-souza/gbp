...
class LagartaResults(DetailView):
    ...
    def post(self, request):
        ...
        gene_list = []
        genes = []
        nextgene = Lagarta_Gene_Information()
        if not not gene:
            try:
                if organism != '0':
                    genes = Lagarta_Gene_Information.objects.filter(gene_name__icontains=gene, organism_id=int(organism))
                    #context['genes'] = genes
                else:
                    genes = Lagarta_Gene_Information.objects.filter(gene_name__icontains=gene)
                    #context['genes'] = genes
        ...
        if not not gene_function:
            try:
                if organism != '0':
                    genes = Lagarta_Gene_Information.objects.filter(gene_description__icontains=gene_function, organism_id=int(organism))
                    #context['genes'] = genes
                else:
                    genes = Lagarta_Gene_Information.objects.filter(gene_description__icontains=gene_function)
                    #context['genes'] = genes
            except ObjectDoesNotExist:
                nextgene.gene_name = 'Busca retornou 0 genes'
                genes.append(nextgene)
        ...
        if not not go_function:
            try:
                if organism != '0':
                    genes = Lagarta_Gene_Information.objects.filter(gene_ontology_blastx__icontains=go_function, organism_id=int(organism))
                    #context['gene_ontology_blastx'] = newgene
                else:
                    genes = Lagarta_Gene_Information.objects.filter(gene_ontology_blastx__icontains=go_function)
        ...
        for g in gene_list:
            expression = []
            try:
                expression = Lagarta_Expression.objects.filter(gene=int(g.id))
            except ObjectDoesNotExist:
                expression = []
            if(len(expression)>1):
            ...
            dsRNAs = []
            try:
                dsRNAs = Lagarta_Dsrna_Information.objects.filter(gene=int(g.id))
                    #context['gene_ontology_blastx'] = newgene
            except ObjectDoesNotExist:
                dsRNAs = []
            ...
            if not not dsRNAs:
                for ds in dsRNAs:
                    iscore = []
                    try:
                        iscore = Lagarta_Iscore.objects.filter(dsrna=int(ds.id))
                    except ObjectDoesNotExist:
                        iscore = []
                    ...
                    dicer = []
                    try:
                        dicer = Lagarta_Dicer.objects.filter(dsrna=int(ds.id))
                    except ObjectDoesNotExist:
                        dicer = []
                    ...
                    estrutura = []
                    try:
                        estrutura = Lagarta_Structure.objects.filter(dsrna=int(ds.id))
                    except ObjectDoesNotExist:
                        estrutura = []
